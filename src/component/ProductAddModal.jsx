...
const ProductAddModal = ({ setOpenModal, openModal }) => {
    ...
    return (
        <div onClick={() => setOpenModal(false)} className={`fixed flex justify-center items-center z-[100] ${openModal ? 'visible opacity-1' : 'invisible opacity-0'} inset-0 w-full h-full backdrop-blur-sm bg-black/20 duration-100`}>
            <div onClick={(e_) => e_.stopPropagation()} className={`absolute w-full lg:w-[500px] bg-white border-b-4 border-blue-500 drop-shadow-2xl rounded-lg ${openModal ? 'opacity-1 duration-300 translate-y-0' : '-translate-y-20 opacity-0 duration-150'}`}>
                <form onSubmit={handleSubmit(onSubmit)} className="p-12">
                    ...
                    <h1 className="backdrop-blur-sm text-4xl pb-3">Add Product</h1>
                    <div className="space-y-3">
                        <label className="block">Book name</label>
                        <div className="relative">
                            <input {...register("bookName")}  type="text" placeholder="product name" className="p-3 block w-full pl-10 drop-shadow-lg rounded-lg outline-none" />
                        </div>
                        <label className="block">Description</label>
                        <div className="relative">
                            <input {...register("descriptions")} type="text" placeholder="Description" className="p-3 block w-full pl-10 drop-shadow-lg rounded-lg outline-none" />
                        </div>
                        <label className="block">Book Photo</label>
                       <input {...register('bookImage')} type="file" />
                    </div>
                    ...
                    <button type="submit" className="py-2 px-10 text-white font-bold text-xl mb-4 mt-6 shadow-lg flex ju rounded-lg before:block before:-left-1 before:-top-1 before:bg-blue-500 before:rounded-lg before:absolute before:h-0 before:w-0 before:hover:w-[100%] before:hover:h-[100%]  before:duration-500 before:-z-40 after:block after:-right-1 after:-bottom-1 after:bg-blue-500 after:rounded-lg after:absolute after:h-0 after:w-0 after:hover:w-[100%] after:hover:h-[100%] after:duration-500 after:-z-40 bg-amber-400 relative inline-block">
                        {status === 'pending'?<ButtonLoader/>:"Submit"}</button>
